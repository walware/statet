...
public class RCommandDirectLaunchShortcut implements ILaunchShortcut {
	...
	private void doLaunch(final IEditorInput editorInput, final ITextSelection selection, final IProgressMonitor monitor)
		...
		RAstNode[] nodes = RAst.findDeepestCommands(astInfo.root, selection.getOffset(), selection.getOffset()+selection.getLength());
		if (nodes == null || nodes.length == 0) {
			return;
		}
		String[] lines = LaunchShortcutUtil.listLines(doc, new TextSelection(doc, nodes[0].getStartOffset(), nodes[nodes.length-1].getStopOffset()-nodes[0].getStartOffset()));
