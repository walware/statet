...
public class ExtractTempHandler extends AbstractHandler {
	
	
	public ExtractTempHandler() {
	}
	...
	public Object execute(final ExecutionEvent event) throws ExecutionException {
		...
		if (selection instanceof ITextSelection) {
			...
			refactoring = new ExtractTempRefactoring(su, new Region(textSelection.getOffset(), textSelection.getLength()));
		}
		if (refactoring != null) {
			final RefactoringWizardExecutionHelper executionHelper = new RefactoringWizardExecutionHelper(
					new ExtractTempWizard(refactoring), RefactoringSaveHelper.SAVE_NOTHING);
...
//						Messages.ExtractTemp_Wizard_title, e),
