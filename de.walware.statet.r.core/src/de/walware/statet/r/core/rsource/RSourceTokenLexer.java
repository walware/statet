...
public class RSourceTokenLexer extends RLexer {
	...
	protected void createFix(final RTerminal type) {
		fNextToken = RSourceToken.createFix(type, fNextIndex, fInput.getLength(fNextNum), STATUS_OK);
	}
	
	@Override
	protected void createSpecialToken(final int status) {
		fNextToken = RSourceToken.create(RTerminal.SPECIAL, fNextIndex, fInput.getLength(fNextNum), fInput.substring(1, fNextNum), status);
	}
	
	@Override
	protected void createSymbolToken() {
		fNextToken = RSourceToken.create(RTerminal.SYMBOL, fNextIndex, fInput.getLength(fNextNum), fInput.substring(1, fNextNum), STATUS_OK);
	}
	
	@Override
	protected void createQuotedSymbolToken(final RTerminal type, final int status) {
		fNextToken = RSourceToken.create(type, fNextIndex, fInput.getLength(fNextNum), fInput.substring(1, fNextNum), status);
	}
	...
	protected void createWhitespaceToken() {
		fNextToken = RSourceToken.create(RTerminal.BLANK, fNextIndex, fInput.getLength(fNextNum), fInput.substring(1, fNextNum), STATUS_OK);
	}
	
	@Override
	protected void createCommentToken(final RTerminal type) {
		fNextToken = RSourceToken.create(type, fNextIndex, fInput.getLength(fNextNum), fInput.substring(1, fNextNum), STATUS_OK);
	}
	
	@Override
	protected void createLinebreakToken(final String text) {
		fNextToken = RSourceToken.create(RTerminal.LINEBREAK, fNextIndex, fInput.getLength(fNextNum), text, STATUS_OK);
	}
	
	@Override
	protected void createUnknownToken(final String text) {
		fNextToken = RSourceToken.create(RTerminal.UNKNOWN, fNextIndex, fInput.getLength(fNextNum), text, STATUS_OK);
	}
